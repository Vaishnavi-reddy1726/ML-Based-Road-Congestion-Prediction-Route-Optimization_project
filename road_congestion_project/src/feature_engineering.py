...
def add_congestion_class(df: pd.DataFrame) -> pd.DataFrame:
    """
    Discretizes the continuous congestion_level (0-1) into 3 operational
    levels used for the classification model: Low / Medium / High.
    Thresholds are fixed (not quantile-based) so they have a stable,
    interpretable real-world meaning across any slice of data.
    """
    df = df.copy()
    bins = [-0.01, 0.33, 0.66, 1.01]
    labels = ["Low", "Medium", "High"]
    df["congestion_class"] = pd.cut(df["congestion_level"], bins=bins, labels=labels)
    return df
